...
class ComplexComparison(object):
    ...
    def __init__(self, gold_standard=[], clusters=[], exclusion_complexes=[], samples=10000, pseudocount=1, exact=False, max_clique=10, remove_non_gold_standard_proteins=False, normalize_by_combinations=False):
    ...
    def clique_comparison_metric_grandf1score(self,mean_func = hmean):
        ...
        f1score_list = [result_dict[x]['f1score'] for x in list(result_dict.keys())]
    ...
    def clique_comparison_metric_mean(self, weighted=False):
        result_dict = self.clique_comparison_metric()
        precision_list = [result_dict[x]['precision'] for x in list(result_dict.keys())]
        recall_list = [result_dict[x]['recall'] for x in list(result_dict.keys())]
        ...
        if weighted:
            sum_complexes = sum([result_dict[x]['numOfClusters'] for x in list(result_dict.keys())])
            weights_list = [1.0*result_dict[x]['numOfClusters']/sum_complexes for x in list(result_dict.keys())]
    ...
    def clique_comparison_exact(self,clique_size):
        ...
        for clust in clusters:
            is_positive_list = [ np.max(list(map(set(group).issubset,self.get_gold_standard()))) for group in it.combinations(clust, clique_size)]
        ...
        for gs_clust in self.get_gold_standard():
            is_positive_list = [ np.max(list(map(set(gs_group).issubset,clusters))) for gs_group in it.combinations(gs_clust, clique_size) ]
    ...
    def clique_comparison(self, clique_size):
        ...
        for random_clique in random_cliques:
            ...
            if np.max(list(map(random_clique.issubset,self.get_gold_standard()))):
                ...
                if len(self.get_exclusion_complexes()) > 0 and np.max(list(map(random_clique.issubset,self.get_exclusion_complexes()))):
...
def main():
    ...
    parser.add_argument("--max_clique", action="store", type=int, dest="max_clique", required=True, help="Maximum number of cliques to check")
    ...
    cplx_compare = ComplexComparison(gold_standard_complexes, predicted_clusters, remove_non_gold_standard_proteins=args.remove_non_gold_standard_proteins, exclusion_complexes=excluded_complexes, normalize_by_combinations=args.normalize_by_combinations, pseudocount=args.pseudocount, max_clique=args.max_clique)
